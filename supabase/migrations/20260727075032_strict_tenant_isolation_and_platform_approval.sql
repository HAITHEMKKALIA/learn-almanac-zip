...
-- Every non-platform action is scoped to an active school and an approved
-- membership. The global `admin` role is no longer a cross-tenant bypass.

-- ---------------------------------------------------------------------------
-- 1. Tenant-scoped roles and approval metadata
-- ---------------------------------------------------------------------------
...
-- Backward-compatible global policies may still ask whether somebody is an
-- `admin`. Only the platform owner (super_admin) may receive global access.
...
AS $$
  SELECT CASE
    WHEN _role = 'admin'::public.app_role
      THEN public.is_super_admin(_user_id)
    ELSE EXISTS (
      SELECT 1
  ...
  SELECT public.is_super_admin(_user_id)
    OR EXISTS (
      ...
      WHERE sm.school_id = _school_id
        ...
        AND COALESCE(p.approved, false)
        AND (
          sm.role = 'owner'::public.school_role
          OR sm.space_role = 'school_admin'::public.app_role
        )
    )
$$;
      ...
      WHERE sm.school_id = _school_id
        ...
        AND (
          ...
          OR sm.space_role IN (
            ...
            'academic_director'::public.app_role,
            'pedagogical_coordinator'::public.app_role,
            'examiner'::public.app_role,
  ...
  SELECT public.is_super_admin(_user_id)
    OR EXISTS (
      ...
      WHERE s.id = _school_id
        ...
        AND s.tenant_type = 'independent_teacher'
        AND s.status = 'active'
        AND sm.status = 'approved'
        AND COALESCE(p.approved, false)
    )
...
AS $$
  SELECT public.is_super_admin(_user_id)
    OR EXISTS (
      ...
      WHERE s.id = _school_id
        AND s.owner_id = _user_id
        AND s.tenant_type = 'independent_student'
        AND s.status = 'active'
        AND sm.status = 'approved'
        AND COALESCE(p.approved, false)
    )
...
SET search_path = public
AS $$
  SELECT public.is_school_member(_school_id, _user_id)
$$;
...
AS $$
  SELECT public.is_super_admin(_left_user)
    ...
    OR EXISTS (
      SELECT 1
      FROM public.school_members viewer
      JOIN public.school_members target
      ...
      WHERE viewer.user_id = _left_user
        AND target.user_id = _right_user
        AND viewer.status = 'approved'
        AND target.status = 'approved'
        AND s.status = 'active'
        AND COALESCE(vp.approved, false)
        AND COALESCE(tp.approved, false)
    )
$$;
...
-- ---------------------------------------------------------------------------
-- 3. Only active and approved spaces are returned to the application
-- ---------------------------------------------------------------------------
...
AS $$
  ...
  ORDER BY
    ...
    CASE COALESCE(sm.space_role::text, sm.role::text)
      ...
      WHEN 'pedagogical_coordinator' THEN 3
      WHEN 'teacher' THEN 4
...
-- ---------------------------------------------------------------------------
-- 4. Core RLS: schools, memberships, classes, messages and presence
-- ---------------------------------------------------------------------------
...
CREATE POLICY schools_select_isolated
ON public.schools FOR SELECT TO authenticated
USING (
  public.is_super_admin((SELECT auth.uid()))
  OR public.is_school_member(id, (SELECT auth.uid()))
  OR owner_id = (SELECT auth.uid())
);

CREATE POLICY schools_insert_platform_only
ON public.schools FOR INSERT TO authenticated
WITH CHECK (public.is_super_admin((SELECT auth.uid())));

CREATE POLICY schools_update_platform_or_active_owner
ON public.schools FOR UPDATE TO authenticated
USING (
  public.is_super_admin((SELECT auth.uid()))
  OR public.is_school_owner(id, (SELECT auth.uid()))
)
WITH CHECK (
  public.is_super_admin((SELECT auth.uid()))
  OR (
    public.is_school_owner(id, (SELECT auth.uid()))
    AND status = 'active'
  )
);

CREATE POLICY schools_delete_platform_only
ON public.schools FOR DELETE TO authenticated
USING (public.is_super_admin((SELECT auth.uid())));

CREATE POLICY memberships_select_isolated
...
CREATE POLICY memberships_request_self
...
CREATE POLICY memberships_invite_pending
ON public.school_members FOR INSERT TO authenticated
WITH CHECK (
  public.is_school_owner(school_id, (SELECT auth.uid()))
  AND status = 'pending'
  AND approved_by IS NULL
  AND approved_at IS NULL
);

CREATE POLICY memberships_review_platform_only
...
USING (
  public.is_super_admin((SELECT auth.uid()))
  OR user_id = (SELECT auth.uid())
  OR public.is_school_owner(school_id, (SELECT auth.uid()))
);

CREATE POLICY classes_select_isolated
...
CREATE POLICY classes_insert_isolated
...
CREATE POLICY classes_update_isolated
...
CREATE POLICY classes_delete_isolated
...
CREATE POLICY class_members_select_isolated
  ...
  OR EXISTS (
    ...
    WHERE c.id = class_members.class_id
      AND (
        ...
        OR public.is_school_teacher(c.school_id, (SELECT auth.uid()))
      )
  )
);

CREATE POLICY class_members_insert_isolated
  ...
  EXISTS (
    ...
    WHERE c.id = class_members.class_id
      ...
      AND (
        public.is_super_admin((SELECT auth.uid()))
        OR c.teacher_id = (SELECT auth.uid())
        OR public.is_school_owner(c.school_id, (SELECT auth.uid()))
      )
  )
);

CREATE POLICY class_members_delete_isolated
...
CREATE POLICY messages_select_participants_same_tenant
...
USING (
  ...
  AND public.users_share_school(sender_id, recipient_id)
);

CREATE POLICY messages_insert_same_tenant
ON public.direct_messages FOR INSERT TO authenticated
WITH CHECK (
  sender_id = (SELECT auth.uid())
  AND public.is_approved((SELECT auth.uid()))
  AND public.users_share_school(sender_id, recipient_id)
);
...
CREATE POLICY presence_manage_self
ON public.user_presence FOR ALL TO authenticated
USING (user_id = (SELECT auth.uid()))
WITH CHECK (user_id = (SELECT auth.uid()));

CREATE POLICY presence_select_shared_tenant
ON public.user_presence FOR SELECT TO authenticated
USING (
  public.users_share_school((SELECT auth.uid()), user_id)
);
...
USING (
  ...
  OR (
    ...
    AND public.is_school_owner(school_id, (SELECT auth.uid()))
  )
);
...
CREATE POLICY audit_select_isolated
...
-- Global admin policies on profiles, roles and consent are platform-owner only.
...
CREATE POLICY profiles_platform_delete
ON public.profiles FOR DELETE TO authenticated
USING (public.is_super_admin((SELECT auth.uid())));

CREATE POLICY roles_platform_manage
ON public.user_roles FOR ALL TO authenticated
USING (public.is_super_admin((SELECT auth.uid())))
WITH CHECK (public.is_super_admin((SELECT auth.uid())));

CREATE POLICY consent_platform_select
ON public.consent_logs FOR SELECT TO authenticated
USING (public.is_super_admin((SELECT auth.uid())));

-- Official curriculum and vocabulary are global. Schools may read them but
-- only the platform owner may publish or modify them.
...
CREATE POLICY kapitel_platform_write
ON public.kapitel FOR ALL TO authenticated
USING (public.is_super_admin((SELECT auth.uid())))
WITH CHECK (public.is_super_admin((SELECT auth.uid())));

CREATE POLICY kapitel_sections_platform_write
ON public.kapitel_sections FOR ALL TO authenticated
USING (public.is_super_admin((SELECT auth.uid())))
WITH CHECK (public.is_super_admin((SELECT auth.uid())));

CREATE POLICY vocab_themes_platform_write
ON public.vocab_themes FOR ALL TO authenticated
USING (public.is_super_admin((SELECT auth.uid())))
WITH CHECK (public.is_super_admin((SELECT auth.uid())));

CREATE POLICY vocab_entries_platform_write
ON public.vocab_entries FOR ALL TO authenticated
USING (public.is_super_admin((SELECT auth.uid())))
WITH CHECK (public.is_super_admin((SELECT auth.uid())));

-- Enforce the tenant boundary for every public table carrying school_id, even
-- if a legacy permissive write policy is still present.
...
DECLARE
  ...
  _old_school_id uuid;
BEGIN
  IF _uid IS NULL OR public.is_super_admin(_uid) THEN
    RETURN NEW;
  END IF;
  ...
  IF TG_TABLE_NAME = 'school_members'
     AND to_jsonb(NEW)->>'user_id' = _uid::text
     AND to_jsonb(NEW)->>'status' = 'pending'
  THEN
    RETURN NEW;
  END IF;
...
BEGIN
  ...
  LOOP
    EXECUTE format(
      'DROP TRIGGER IF EXISTS enforce_school_tenant_write ON public.%I',
      table_record.table_name
    );
    EXECUTE format(
      'CREATE TRIGGER enforce_school_tenant_write
         BEFORE INSERT OR UPDATE ON public.%I
         FOR EACH ROW EXECUTE FUNCTION public.enforce_school_tenant_write()',
      table_record.table_name
    );
  END LOOP;
END
...
-- ---------------------------------------------------------------------------
-- 5. Platform-owner review workflow
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  SELECT
    ...
    sm.school_id,
    sm.user_id,
    COALESCE(sm.space_role::text, sm.role::text),
    sm.status,
    sm.joined_at,
    s.name,
    p.display_name,
...
CREATE OR REPLACE FUNCTION public.platform_review_school(
  _school_id uuid,
  _decision text,
...
BEGIN
  IF auth.uid() IS NULL OR NOT public.is_super_admin(auth.uid()) THEN
    RAISE EXCEPTION 'platform_owner_required' USING ERRCODE = '42501';
  END IF;
  IF _decision NOT IN ('approve', 'reject', 'suspend', 'archive') THEN
  ...
  IF _decision = 'approve' THEN
    ...
    SELECT
      ...
      CASE s.tenant_type
        ...
        ELSE 'school_admin'::public.app_role
      END
    FROM public.schools s
    WHERE s.id = _school_id
    ON CONFLICT (user_id, role) DO NOTHING;
  ELSE
    UPDATE public.school_members
    SET status = CASE WHEN _decision = 'suspend' THEN 'suspended' ELSE 'rejected' END,
        reviewed_at = now(),
        review_reason = NULLIF(trim(_reason), '')
    WHERE school_id = _school_id;
  END IF;
...
CREATE OR REPLACE FUNCTION public.platform_review_membership(
  _membership_id uuid,
  _decision text,
  _reason text DEFAULT NULL
)
...
BEGIN
  IF auth.uid() IS NULL OR NOT public.is_super_admin(auth.uid()) THEN
    RAISE EXCEPTION 'platform_owner_required' USING ERRCODE = '42501';
  END IF;
  IF _decision NOT IN ('approve', 'reject', 'suspend') THEN
    RAISE EXCEPTION 'invalid_decision';
  END IF;
  ...
  SELECT status INTO _school_status
  FROM public.schools
  WHERE id = _membership.school_id;
  IF _decision = 'approve' AND _school_status <> 'active' THEN
    RAISE EXCEPTION 'school_must_be_active_before_member_approval';
  END IF;
  ...
  IF _decision = 'approve' THEN
    UPDATE public.profiles
    SET approved = true, updated_at = now()
    WHERE user_id = _membership.user_id;

    IF _membership.space_role IS NOT NULL THEN
      INSERT INTO public.user_roles(user_id, role)
      VALUES (_membership.user_id, _membership.space_role)
      ON CONFLICT (user_id, role) DO NOTHING;
    END IF;
    ...
    IF _membership.requested_class_id IS NOT NULL
       AND EXISTS (
         SELECT 1
         FROM public.classes c
         WHERE c.id = _membership.requested_class_id
           AND c.school_id = _membership.school_id
       )
    THEN
      INSERT INTO public.class_members(class_id, student_id)
      VALUES (_membership.requested_class_id, _membership.user_id)
  ...
  VALUES (
    auth.uid(),
    'platform.review_membership',
    'school_members',
    _membership_id,
    _membership.school_id,
    jsonb_build_object(
      'decision', _decision,
      'reason', _reason,
...
AS $$
DECLARE
  _legacy_role public.school_role;
  _space_role public.app_role;
  _member_id uuid;
BEGIN
  IF auth.uid() IS NULL OR NOT public.is_super_admin(auth.uid()) THEN
    RAISE EXCEPTION 'platform_owner_required' USING ERRCODE = '42501';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM public.schools WHERE id = _school_id AND status = 'active') THEN
    RAISE EXCEPTION 'active_school_required';
  END IF;
  IF _role = 'super_admin'::public.app_role THEN
    RAISE EXCEPTION 'super_admin_assignment_forbidden';
  END IF;

  _space_role := CASE
    WHEN _role = 'admin'::public.app_role THEN 'school_admin'::public.app_role
    ELSE _role
  END;
  _legacy_role := CASE
    WHEN _space_role IN (
      ...
      'teacher'::public.app_role,
      'staff'::public.app_role
    ) THEN 'teacher'::public.school_role
  ...
  IF _member_id IS NULL THEN
    INSERT INTO public.school_members(
      school_id, user_id, role, space_role, status, approved_by, approved_at, requested_class_id
    )
    VALUES (
      _school_id, _target, _legacy_role, _space_role, 'approved', auth.uid(), now(), _class_id
    )
    RETURNING id INTO _member_id;
  ELSE
    UPDATE public.school_members
    SET role = _legacy_role,
  ...
  IF _class_id IS NOT NULL AND _space_role = 'student'::public.app_role THEN
    IF NOT EXISTS (
      ...
      WHERE id = _class_id AND school_id = _school_id
    ) THEN
...
BEGIN
  ...
  VALUES (
    auth.uid(),
    'platform.review_profile',
    'profiles',
    _target,
    jsonb_build_object('approved', _approved)
  );
END;
$$;
...
BEGIN
  IF auth.uid() IS NULL
     OR NOT (
       ...
       OR public.is_school_teacher(_school_id, auth.uid())
     )
  THEN
  ...
  SELECT
    ...
    sm.role,
    CASE
      WHEN sm.space_role IS NULL THEN ARRAY[sm.role::text]
      ELSE ARRAY[sm.space_role::text]
    END AS app_roles,
    COALESCE(
...
CREATE OR REPLACE FUNCTION public.school_assign_student_to_class(
  _school_id uuid,
  _target uuid,
  _class_id uuid
)
...
BEGIN
  IF auth.uid() IS NULL
     OR NOT (
       public.is_super_admin(auth.uid())
       OR public.is_school_owner(_school_id, auth.uid())
     )
  THEN
    RAISE EXCEPTION 'school_manager_required' USING ERRCODE = '42501';
  END IF;
  IF NOT public.is_school_member(_school_id, _target) THEN
    RAISE EXCEPTION 'approved_school_member_required';
  END IF;
  IF NOT EXISTS (
    SELECT 1
    FROM public.classes c
  ...
  VALUES (
    auth.uid(),
    'school.assign_student_to_class',
    'classes',
    _class_id,
    _school_id,
    jsonb_build_object('student_id', _target)
  );
END;
$$;
...
-- ---------------------------------------------------------------------------
-- 6. Requests: school, independent spaces and class joining
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  END IF;
  IF NOT public.is_approved(_uid) THEN
    RAISE EXCEPTION 'profile_approval_required';
  END IF;
  ...
  INSERT INTO public.schools(
    name, slug, owner_id, status, tenant_type, is_independent
  )
  VALUES (_name, _slug, _uid, 'pending', 'school', false)
  RETURNING id INTO _school_id;

  INSERT INTO public.school_members(
    school_id, user_id, role, space_role, status
  )
  VALUES (
    _school_id, _uid, 'owner'::public.school_role, 'school_admin'::public.app_role, 'pending'
  );
...
CREATE OR REPLACE FUNCTION public.create_independent_teacher_space(
  _studio_name text,
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE owner_id = _uid
      AND tenant_type = 'independent_teacher'
  ...
  INSERT INTO public.schools(
    name, slug, owner_id, status, tenant_type, is_independent, legal_name
  )
  VALUES (
    _name, _slug, _uid, 'pending', 'independent_teacher', true,
    COALESCE(NULLIF(trim(_display_name), ''), _name)
  )
  RETURNING id INTO _school_id;
  ...
  VALUES (
    _school_id, _uid, 'owner'::public.school_role, 'teacher'::public.app_role, 'pending'
  );

  INSERT INTO public.teacher_studio_settings(school_id, teacher_id, studio_name)
  VALUES (_school_id, _uid, _name)
...
CREATE OR REPLACE FUNCTION public.create_independent_student_space(
  _current_level text DEFAULT 'A1.1'
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM public.schools
    WHERE owner_id = _uid
      AND tenant_type = 'independent_student'
      AND status IN ('pending', 'active')
  ) THEN
  ...
  VALUES (
    'Apprentissage personnel', _slug, _uid, 'pending', 'independent_student', true
  )
  RETURNING id INTO _school_id;
  ...
  VALUES (
    _school_id, _uid, 'student'::public.school_role, 'student'::public.app_role, 'pending'
  );
...
BEGIN
  ...
  INSERT INTO public.school_members(
    school_id, user_id, role, space_role, status, requested_class_id
  )
  VALUES (
    _school_id, _uid, 'student'::public.school_role, 'student'::public.app_role,
    'pending', _class_id
  )
...
BEGIN
  IF _uid IS NULL THEN
    RAISE EXCEPTION 'not_authenticated';
  END IF;
  IF _terms_version <> '2.0' OR _privacy_version <> '2.0' THEN
    RAISE EXCEPTION 'outdated_legal_version';
  END IF;

  INSERT INTO public.consent_logs(
    user_id, consent_type, granted, version, user_agent, metadata
  )
  VALUES
    (_uid, 'terms', true, _terms_version, _user_agent, jsonb_build_object('source', 'legal_gate')),
...
-- Capture consent supplied during email/password sign-up, including when email
-- confirmation means there is not yet a client session.
...
AS $$
DECLARE
  _is_platform_owner boolean := lower(NEW.email) = 'haithem.kalia@gmail.com';
...
BEGIN
  ...
  IF _terms_version = '2.0' AND _privacy_version = '2.0' THEN
    ...
    INSERT INTO public.consent_logs(
      user_id, consent_type, granted, version, metadata
    )
    VALUES
      (NEW.id, 'terms', true, _terms_version, jsonb_build_object('source', 'signup')),
  ...
  IF lower(COALESCE(NEW.raw_user_meta_data->>'guardian_consent', 'false')) = 'true'
     AND NULLIF(NEW.raw_user_meta_data->>'guardian_email', '') IS NOT NULL
  THEN
    INSERT INTO public.consent_logs(
      user_id, consent_type, granted, version, metadata
    )
    VALUES (
      ...
      jsonb_build_object(
        'source', 'signup',
        'guardian_email', NEW.raw_user_meta_data->>'guardian_email'
      )
    );
  END IF;
...
-- New Supabase projects no longer auto-expose new relations. Keep explicit
-- grants bundled with RLS so future deployments remain deterministic.
