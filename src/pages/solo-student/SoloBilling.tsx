...
export default function SoloBilling() {
  const { user } = useAuth();
  ...
  return (
    <SchoolLayout title="Mon abonnement" subtitle="Suivez votre essai gratuit, votre plan et sa date d'expiration.">
      <div className="max-w-3xl mx-auto space-y-6">
        <SubscriptionCard
          ownerUserId={user?.id ?? null}
          schoolId={activeSchool?.id ?? null}
          trialEndsAt={trialEndsAt}
          title="Mon abonnement personnel"
        />

        <Card className="p-5">
          <h3 className="font-semibold mb-3">Plans disponibles</h3>
          {plans.length === 0 ? (
          ...
          <p className="text-xs text-muted-foreground mt-3">
            Pour souscrire ou renouveler, contactez le support — les paiements sont traités manuellement (TND).
          </p>
        </Card>
      </div>
    </SchoolLayout>
